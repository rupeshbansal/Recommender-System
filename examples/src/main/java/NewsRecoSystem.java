        ...
        try {
            ...
            if(flag == 0) {
                for (int i = 0; i < length; i++) {
                    ...
                    String newsId = String.valueOf(json.getJSONObject(i).get("newsId"));
                    String str = Id + "," + newsId + "," + rating;
                    itemDetails.add(str);
                }
            }
//            else if(flag == 1){
...
//            }
    ...
    public static JSONArray newsReco() throws IOException, TasteException, JSONException {
...
//        try {
//            //Create connection
//            url = new URL("http://10.132.235.67:3000/postNLogs");
...
//    }
    ...
    private static JSONArray convertRecoToJson(ArrayList<ArrayList<Integer>> list) throws JSONException, IOException {
        ...
        FileWriter file = new FileWriter("/home/rupesh/RecommendedFiles/RecommendationNews");
        try {
            for(int i = 0 ; i < l ; i++) {
...
//Fetch List of ItemIds and List of ratings.
    private static void populateList(Integer userId, String category) throws IOException {
        String s = "http://10.132.235.67:3000/NLogsCatUser/" + category + "/" + userId;
        ArrayList<String> details = fetchData(s , 0);
        int l = details.size();
        for(int i = 0 ; i < l ; i++){
            String token[] = details.get(i).split(",");
            itemIds.add(Integer.parseInt(token[1]));
    ...
    private static File getAllVideosForCategory(String category) throws IOException {
        ...
        ArrayList<String> list = fetchData("http://10.132.235.67:3000/NLogsCat/" + category , 0);

      //  ArrayList<String> itemDetails = fetchItemsForUser(1,list);
        File file = new File("NewsInCategory.csv");
        file.createNewFile();
        FileWriter writer = new FileWriter(file);
        int l = list.size();
        for(int i = 0 ; i < l ; i++){
            String token[] = list.get(i).split(",");
            String s = token[0] + "," + token[1] + "," + token[2];
           writer.write(s + "\n");
        }
        writer.flush();
        writer.close();
    ...
    private static ArrayList<Integer> getAllUsers() throws IOException {
        ArrayList<String> a = fetchData("http://10.132.235.67:3000/NUsers" , 2);
